...
function DoEntireConePanel(obj) 
{
    ...
    {
        if (head_nptr.Paths == null || head_nptr.Paths.length == 0)
        {
            continue;
        }

        let nclass = head_nptr.RootNode.Class;
        let ncptr = head_nptr.RootNode.CPtr;
...
function DoPageMapPanel(obj) 
{
    ...
    let chapter_name = document.createElement("span");
    chapter_name.textContent = obj.Content.Title;
    panel.appendChild(chapter_name);
...
function DoTOCPanel(obj) 
{
    ...
    for (let chpblk of obj.Content) 
    {
        ...
        let link = document.createElement("a");
        let item = document.createElement("h2");
        link.onclick = function () { sendLinkSearch("\\notes \\chapter \"" + chpblk.Chapter + "\""); };
...
function PrintLink(parent, radius, stindex, arrow, str, nclass, ncptr, chap, ctx,) 
{
    ...
    // too much box.setAttribute("class", "card-view");
    ...
    else 
    {
        ...
        if (str.length < 20) 
        {
            ...
            text_link.style.paddingInline = "1rem";
        }
    ...
    for (let path = 0; path < array.length; path++) 
    {
        ...
        for (let i = 0; i < array[path].length; i++) 
        {
            if (i == 0)
            {
                ...
                newpath.setAttribute("class", "card-view");
        ...
        for (let i = 0; i < array[line].length; i++) 
        {
            ...
            if (i % 2 == 0)
            {
                ...
                else 
                {
                    ...
                    let text = document.createElement("span");
                    text.setAttribute("class", "line-view");
                    //text.setAttribute("class", "break");
                    //text.style.cssText = "display: inline-block;";
                    text.textContent = str;
                    text_link.appendChild(text);
                    subline.appendChild(text_link);
...
function CheckCones(panel, docpart, name, bwd, fwd) 
{
    if (bwd[0] != null && fwd[0] != null) 
    {
        ...
        link.onclick = function () { sendLinkSearch("\\from " + nptrs + " limit 30"); };
