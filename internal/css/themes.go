...
//go:embed style.css.in
var styleTemplate embed.FS
...
// Theme names served as /{name}.css
var ThemeNames = []string{"style", "dark", "slate", "spaceblue", "red"}

func themeVars() map[string]map[string]string {
	// Port of src/server/css-builder theme maps (subset of keys used in style.css.in).
	mk := func(m map[string]string) map[string]string { return m }

	dark := mk(map[string]string{
	...
	// Default / style theme uses light-leaning tokens from original "style" map
	style := mk(map[string]string{
...
// Ensure builds all theme CSS once.
func Ensure() error {
	once.Do(func() {
		themes = make(map[string][]byte)
		raw, err := styleTemplate.ReadFile("style.css.in")
		if err != nil {
			genErr = fmt.Errorf("read style.css.in: %w", err)
			return
		}
		for name, vars := range themeVars() {
			...
			themes[name] = buf.Bytes()
		}
	})
	return genErr
}

// CSS returns generated CSS for a theme name (without .css).
func CSS(name string) ([]byte, error) {
