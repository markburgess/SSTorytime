// Search CLI helpers (ported from upstream searchN4L).
package main
...
func RunSearchCLI(ctx context.Context, sst SST.PoSST, search SST.SearchParameters, line string) {
	...
	if (context || chapter) && !name && !sequence && !pagenr && !(from || to) {
		...
		ShowMatchingChapter(ctx, sst, search.Chapter, search.Context, maxlimit)
		ShowTime(ctx, sst, search)
		return
	}
	...
	if name && !sequence && !pagenr {
		...
		fmt.Println("------------------------------------------------------------------")
		FindOrbits(ctx, sst, nodeptrs, maxlimit)
		ShowTime(ctx, sst, search)
		return
	}
	...
	if from && to {
		...
		fmt.Println("------------------------------------------------------------------")
		PathSolve(ctx, sst, leftptrs, rightptrs, search.Chapter, search.Context, arrowptrs, sttype, minlimit, maxlimit)
		ShowTime(ctx, sst, search)
		return
	}
	...
	if (name || from || to) && !pagenr && !sequence {
		...
		if nodeptrs != nil {
			fmt.Println("------------------------------------------------------------------")
			CausalCones(ctx, sst, nodeptrs, search.Chapter, search.Context, arrowptrs, sttype, maxlimit)
			ShowTime(ctx, sst, search)
			return
		}
		if leftptrs != nil {
			fmt.Println("------------------------------------------------------------------")
			CausalCones(ctx, sst, leftptrs, search.Chapter, search.Context, arrowptrs, sttype, maxlimit)
			ShowTime(ctx, sst, search)
			return
		}
		if rightptrs != nil {
			fmt.Println("------------------------------------------------------------------")
			CausalCones(ctx, sst, rightptrs, search.Chapter, search.Context, arrowptrs, sttype, maxlimit)
			ShowTime(ctx, sst, search)
			return
		}
	}
	...
	if sequence {
		ShowStories(ctx, sst, nodeptrs, arrowptrs, sttype, maxlimit)
		ShowTime(ctx, sst, search)
		return
	}
	...
	if arrows || sttypes {
		ShowMatchingArrows(ctx, sst, arrowptrs, sttype)
		ShowTime(ctx, sst, search)
		return
	}
...
func FindOrbits(ctx context.Context, sst SST.PoSST, nptrs []SST.NodePtr, limit int) {
	...
	for nptr := range nptrs {
		...
		fmt.Print("\n", nptr, ": ")
		SST.PrintNodeOrbit(ctx, &sst, nptrs[nptr], limit)
	}
}

//******************************************************************

func CausalCones(ctx context.Context, sst SST.PoSST, nptrs []SST.NodePtr, chap string, context []string, arrows []SST.ArrowPtr, sttype []int, limit int) {
	...
	for n := range nptrs {
		for st := range sttype {
			...
			fcone, npaths := SST.GetFwdPathsAsLinks(ctx, &sst, nptrs[n], sttype[st], limit, maxlimit)
			if npaths == 0 {
				continue
			}
			...
			if fcone != nil {
				fmt.Printf("%d. ", total)
				total += ShowCone(ctx, sst, fcone, chap, context, limit)
			}
			...
			if sttype[st] != 0 {
				bcone, npaths := SST.GetFwdPathsAsLinks(ctx, &sst, nptrs[n], -sttype[st], limit, maxlimit)
				if npaths == 0 {
					continue
				}
				...
				if bcone != nil {
					fmt.Printf("%d. ", total)
					total += ShowCone(ctx, sst, bcone, chap, context, limit)
				}
...
func PathSolve(ctx context.Context, sst SST.PoSST, leftptrs, rightptrs []SST.NodePtr, chapter string, context []string, arrowptrs []SST.ArrowPtr, sttype []int, mindepth, maxdepth int) {
	var count int
	...
	if len(solutions) > 0 {
		...
		for s := 0; s < len(solutions); s++ {
			...
			PrintConstrainedLinkPath(ctx, sst, solutions, s, prefix, chapter, context, arrowptrs, sttype)
		}
		count++
	}
}
...
func ShowMatchingArrows(ctx context.Context, sst SST.PoSST, arrowptrs []SST.ArrowPtr, sttype []int) {
...
func ShowMatchingChapter(ctx context.Context, sst SST.PoSST, chap string, context []string, limit int) {
...
func ShowChapterContexts(ctx context.Context, sst SST.PoSST, chap string, context []string, limit int) {
...
func ShowStories(ctx context.Context, sst SST.PoSST, nodeptrs []SST.NodePtr, arrowptrs []SST.ArrowPtr, sttypes []int, limit int) {
...
func ShowCone(ctx context.Context, sst SST.PoSST, cone [][]SST.Link, chap string, context []string, limit int) int {
	...
	for s := 0; s < len(cone) && s < limit; s++ {
		SST.PrintSomeLinkPath(ctx, &sst, cone, s, " - ", chap, context, limit)
		count++
	}
...
func PrintConstrainedLinkPath(ctx context.Context, sst SST.PoSST, cone [][]SST.Link, p int, prefix string, chapter string, context []string, arrows []SST.ArrowPtr, sttype []int) {

	for l := 1; l < len(cone[p]); l++ {
		link := cone[p][l]

		if !ArrowAllowed(ctx, sst, link.Arr, arrows, sttype) {
			return
		}
	}

	SST.PrintLinkPath(ctx, &sst, cone, p, prefix, chapter, context)
}

// **********************************************************

func ArrowAllowed(ctx context.Context, sst SST.PoSST, arr SST.ArrowPtr, arrlist []SST.ArrowPtr, stlist []int) bool {

	st_ok := false
	arr_ok := false

	staidx := SST.GetDBArrowByPtr(ctx, &sst, arr).STAindex
...
func ShowNotes(ctx context.Context, sst SST.PoSST, notes []SST.PageMap) {
...
func ShowTime(ctx context.Context, sst SST.PoSST, search SST.SearchParameters) {

	ambient, key, now := SST.GetTimeContext()
	now_ctx := SST.UpdateSTMContext(ctx, &sst, ambient, key, now, search)
